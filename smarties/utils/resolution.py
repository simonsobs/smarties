...
from smarties.utils.tools import (
    get_coupled_spin,
...
def get_chunks_from_shape(
    ...
    if len(array_shape_map) == 1:
        assert np.array(number_chunks).size == 1, (
            "Number of chunks should be a scalar for 1D maps."
        )

        step_pixels = array_shape_map[0] // number_chunks

        list_to_return = [
            (i * step_pixels, (i + 1) * step_pixels) for i in range(number_chunks)
        ]
        if list_to_return[-1][1] < array_shape_map[0]:
            list_to_return = list_to_return + [
                (array_shape_map[0] // step_pixels) * step_pixels,
                array_shape_map[0],
            ]
        return np.int_(list_to_return)

    elif len(array_shape_map) == 2:
        assert np.array(number_chunks).size < 3, (
            "Number of chunks should be a scalar or 2D array for 2D maps."
        )
...
def get_projector_map_resolution(
    ...
    assert projection_pixel in ["healpix", "car"], (
        f"Unknown projection pixel: {projection_pixel}"
    )
...
def get_chunks_final_map_from_downgraded_chunks(
    ...
    assert np.array(shape_final_map).size == np.array(shape_map_downgraded).size, (
        "Shape of the full map and the previous map should have the same number of dimensions."
    )
    ...
    if np.array(shape_final_map).ndim == 1:
        ...
        return np.array(list_chunks_previous_map) * np.array(
            [ratio_x, ratio_x, ratio_y, ratio_y]
        )


def get_slice_1d_from_2d(shape_map, slice_x, slice_y, boolean_mask=None):
...
def ud_grade_hn(h_n_maps, nside_out):
    ...
    for spin in h_n_maps.spins:
        if h_n_maps[spin].ndim != 1 and h_n_maps[spin].shape[-1] != 1:
            number_of_detectors = (
                1 if h_n_maps[spin].ndim == 1 else h_n_maps[spin].shape[0]
            )
            new_h_n[spin] = np.zeros(
                (number_of_detectors, hp.nside2npix(nside_out)),
                dtype=h_n_maps[spin].dtype,
            )
            for detector in range(number_of_detectors):
                new_h_n[spin][detector] = hp.ud_grade(
                    h_n_maps[spin][detector], nside_out, power=None
                )
        else:

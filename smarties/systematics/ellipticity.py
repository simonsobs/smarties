...
def get_differential_ellipticity_BICEP(
    ...
    p_c_parameters_dict = convert_ellipticities_conventions(
        ...
        input_ellipticity_convention=ellipticity_parameters_dict[
            "ellipticity_parameter_convention"
        ],
    ...
    assert parameter_p.shape == parameter_c.shape, (
        "The parameter p and c provided must have the same shape"
    )
    ...
    sigma_cs = (
        np.asarray(sigma_fwhm) / ((8 * np.log(2)) ** 0.5) * np.pi / (180 * 60)
    ...
    if spherical_derivatives is None:
        ...
        if type(intensity_CMB) == np.ndarray:
            ...
            assert intensity_CMB.ndim == 1, (
                "The intensity_CMB map must be a 1D array compatible with a full sky healpy map"
            )
            assert np.log(np.sqrt(intensity_CMB.size / 12)) / np.log(2) % 1 == 0, (
                "The intensity_CMB map dimension must be compatible with a full sky healpy map"
            )
            shape_car = None
        elif type(intensity_CMB) == enmap.ndmap:
            # CAR pixelization expected
            assert shape_car is not None, (
                "The shape_car must be provided if the intensity map is provided as enmap.ndmap"
            )

            if shape_fullsky_car is None:
                shape_fullsky_car = (
                    intensity_CMB.shape if intensity_CMB.ndim == 2 else None
                )
            assert shape_fullsky_car is not None

            assert type(mask) == enmap.ndmap, (
                "The provided mask must be of type ndmap if the intensity map is as well provided with this type"
            )
            box_coordinates = mask.reshape(shape_car).corners()

            take_box_function = lambda x: (
                x.reshape(shape_fullsky_car).submap(box_coordinates).ravel()
            )  # [mask_bool]

        else:
            raise ValueError(
                "The intensity_CMB map must be either a 1D array compatible with a full sky healpy map or a 2D array compatible with a CAR array"
            )

        alms_I = map2alm_anypix(
            intensity_CMB, spin=0, lmax=lmax, niter=10, shape_car=shape_fullsky_car
        )
        ...
        assert "phi_phi" in spherical_derivatives, (
            "The provided spherical_derivatives dictionnary must contain the 'phi_phi' key for the second derivative with respect to phi (including factor 1/sin^2(theta))"
        )
        assert "theta_phi" in spherical_derivatives, (
            "The provided spherical_derivatives dictionnary must contain the 'theta_phi' key for the second derivative with respect to theta and phi"
        )
        assert "theta_theta" in spherical_derivatives, (
            "The provided spherical_derivatives dictionnary must contain the 'theta_theta' key for the second derivative with respect to theta"
        )
        assert "phi" in spherical_derivatives, (
            "The provided spherical_derivatives dictionnary must contain the 'phi' key for the first derivative with respect to phi"
        )
        ...
        for spin in spherical_derivatives:
            if spherical_derivatives[spin].shape != np.prod(shape_car):
                spherical_derivatives[spin] = take_box_function(
                    spherical_derivatives[spin]
                )
        ...
        central_term = (
            ...
            multiply_tan_theta_power(
                spherical_derivatives["phi"], power=-1, shape_car=shape_car
            )[mask_bool]
    ...
    if type(intensity_CMB) == enmap.ndmap:
        for spin in differential_ellipticity_spin_maps.spins:
            differential_ellipticity_spin_maps[spin] = enmap.ndmap(
                differential_ellipticity_spin_maps[spin], wcs=intensity_CMB.wcs
            )
    else:
        for spin in differential_ellipticity_spin_maps.spins:
            differential_ellipticity_spin_maps[spin] = (
                differential_ellipticity_spin_maps[spin].squeeze()
            )
...
def get_differential_ellipticity_no_calibration(
    ...
    assert delta_sigma.ndim == 1, (
        "The parameter delta_sigma rebuilt provided must have shape (n_det)"
    )
    assert ellipticity_angle.ndim == 1, (
        "The parameter ellipticity_angle provided must have shape (n_det)"
    )
    assert delta_sigma.shape == ellipticity_angle.shape, (
        "The parameter p and c provided must have the same shape"
    )
    ...
    propagation_perturbation_ellipse = np.einsum(
        "dxy, xz, dza->dya",
    ...
    crossed_derivatives_imaginary_part = (
        2 * 1j * spherical_derivatives["theta_phi"][mask_bool]
    )

    derivatives_maps = {f"{i}{j}": Spin_maps() for i in ["x", "y"] for j in ["x", "y"]}

    derivatives_maps["xx"][-2] = (
        d2phi_minus_d2theta + crossed_derivatives_imaginary_part
    ) / 4
    derivatives_maps["xx"][0] = 0.5 * (
        spherical_derivatives["phi_phi"][mask_bool]
        + spherical_derivatives["theta_theta"][mask_bool]
    )

    derivatives_maps["yy"][-2] = (
        -d2phi_minus_d2theta - crossed_derivatives_imaginary_part
    ) / 4
    derivatives_maps["yy"][0] = 0.5 * (
        spherical_derivatives["phi_phi"][mask_bool]
        + spherical_derivatives["theta_theta"][mask_bool]
    )

    derivatives_maps["xy"][-2] = (
        -d2phi_minus_d2theta - crossed_derivatives_imaginary_part
    ) / (4 * 1j)
    ...
    for spin in [-2, 0, 2]:
        differential_ellipticity_spin_maps[spin] = np.zeros(
            (delta_sigma.size, npix), dtype=np.complex128
        )

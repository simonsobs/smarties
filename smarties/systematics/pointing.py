...
__all__ = ["create_pointing_spin_leakage_map", "create_pointing_spin_leakage_map_BICEP"]
...
def _build_central_term_pointing_leakage(
    ...
    if spherical_derivatives is None:
        ...
        assert "theta" in spherical_derivatives, (
            "The provided spherical_derivatives dictionnary must contain the 'theta' key for the first derivative with respect to theta"
        )
        ...
        if shape_car is not None:
            for spin in spherical_derivatives:
                if spherical_derivatives[spin].shape != np.prod(shape_car):
                    raise ValueError(
                        f"The provided spherical_derivatives for spin {spin} have a shape {spherical_derivatives[spin].shape} that is not compatible with the expected shape of the flattened CAR maps {np.prod(shape_car)}"
                    )
...
def create_pointing_spin_leakage_map(
    ...
    assert np.array(amplitude_offset).ndim == 1, (
        "The dimension of the amplitude_offset must be (n_det,)"
    )
    assert amplitude_offset.shape == angle_offset.shape, (
        "The amplitude offset must have the same shape as the angle offset"
    )
    ...
    pointing_leakage_spin_maps[-1] = contract(
        "d,p->dp", -amplitude_offset / 2.0 * np.exp(1j * angle_offset), central_term
    )
...
def create_pointing_spin_leakage_map_BICEP(
    ...
    assert delta_x.shape == delta_y.shape, (
        "The offset in x must have the same shape as the offset in y"
    )
    ...
    # Spin -1
    pointing_leakage_spin_maps[-1] = contract(
        "d,p->dp", (delta_x - 1j * delta_y) / 2.0, central_term
    )
